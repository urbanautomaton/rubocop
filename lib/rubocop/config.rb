...
module RuboCop
  ...
  class Config
    ...
    def obsolete_parameters
      obsolete_parameter_config.map do |params|
        obsolete_parameter_message(params[:cop], params[:parameter],
                                   params[:alternative])
      end
    end

    def obsolete_parameter_config
      config_file = File.join(
        ConfigLoader::RUBOCOP_HOME,
        'config',
        'obsolete_parameters.yml'
      )

      YAMLLoader.safe_load(File.read(config_file), config_file)
    end
    ...
    def obsolete_cops
      obsolete_cop_config.map do |cop_name, message|
